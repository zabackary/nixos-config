  ...
  systemd.user.services.freeshow-media-sync = {
    ...
    Service = {
      ...
      ExecStart = pkgs.writeShellScript "freeshow-media-sync.sh" ''
        set -eou pipefail
        ${pkgs.rclone}/bin/rclone bisync --config "$HOME/.config/rclone/rclone.conf" --progress --stats 1s --log-level INFO --log-file "$HOME/.cache/freeshow-media-sync.log" ~/Documents/FreeShow/Media "school_gdrive:Chapel Committee/Chapel Slides/Freeshow Media Sync CAJ"
